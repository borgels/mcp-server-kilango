...
export async function readTextBody(req: IncomingMessage, maxBodyBytes = getHttpConfig().maxBodyBytes): Promise<string> {
  const chunks: Buffer[] = [];
  let totalBytes = 0;
...
export async function readJsonBody(req: IncomingMessage, maxBodyBytes = getHttpConfig().maxBodyBytes): Promise<unknown> {
...
export function sendJson(
  ...
  res.writeHead(status, { ...corsHeaders(req), 'Content-Type': 'application/json', ...extraHeaders });
